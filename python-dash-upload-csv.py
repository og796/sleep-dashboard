...
def load_data(file_path='export.xml'):
    print("Loading data...")
    start_time = time.time()
    
    # First convert XML to CSV with only needed columns
    ...
    # Filter only SleepAnalysis records
    data = data[data['type'].str.contains('SleepAnalysis')].copy()
    
    # Keep only the needed columns
    sleep_data = data[['creationDate', 'startDate', 'endDate', 'value']]
    ...
    # Convert date columns to datetime again since they were saved as strings
    for col in ['creationDate', 'startDate', 'endDate']:
        data[col] = pd.to_datetime(data[col])
...
def run_dashboard():
    ...
    global data, agg, weekly_agg, monthly_agg, merged_data, gantt_data, night_dropdown_options
    data = None
    agg = None
    weekly_agg = None
    monthly_agg = None
    merged_data = None
    gantt_data = None
    night_dropdown_options = []
    
    # Create empty figure for initial state
    empty_fig = go.Figure()
    empty_fig.update_layout(
        title="Upload a file to view data",
        plot_bgcolor="#1a1a1a",
        paper_bgcolor="#1a1a1a",
        font=dict(color="white")
    )
    ...
    app.layout = dbc.Container(
        ...
        children=[
            ...
            dbc.Row([
                dbc.Col([
                    dbc.Card([
                        dbc.CardHeader("Upload Sleep Data"),
                        dbc.CardBody([
                            dcc.Upload(
                                id='upload-data',
                                children=html.Div([
                                    'Drag and Drop or ',
                                    html.A('Select XML or CSV File')
                                ]),
                                style={
                                    'width': '100%',
                                    'height': '60px',
    ...
    @app.callback(
        [Output('dashboard-content', 'style'),
         ...
         Output('night-select', 'options')],
        [Input('upload-data', 'contents')],
        [State('upload-data', 'filename')]
    )
    def update_dashboard_visibility(contents, filename):
        if contents is None:
            return {'display': 'none'}, html.Div("Please upload an XML or CSV file to begin analysis."), []
        ...
        try:
            ...
            if filename.endswith('.xml'):
                ...
                required_cols = ['creationDate', 'startDate', 'endDate', 'value']
                if not all(col in data.columns for col in required_cols):
                    raise ValueError("CSV file must contain columns: creationDate, startDate, endDate, value")
                ...
                # Add type column to match XML format
                data['type'] = 'SleepAnalysis'
                
                # Sort by creationDate to ensure proper processing
                data = data.sort_values('creationDate')
            else:
                raise ValueError("Unsupported file type. Please upload an XML or CSV file.")
            ...
            return (
                {'display': 'block'}, 
                html.Div([
                    html.H5(f"Successfully loaded {filename}"),
                    html.P("Data processing complete. Dashboard is now ready.")
                ]), 
                night_dropdown_options
            )
            ...
            return (
                ...
                html.Div([
                    ...
                    html.P(str(e)),
                    html.P("Please make sure you're uploading a valid XML or CSV file with the correct columns.")
                ]), 
                []
            )
    ...
    def update_graphs(contents, selected_night, selected_metric, week_filter, time_period, smoothing_window):
        from dash import callback_context
        
        # If no file uploaded yet, return empty figures
        if contents is None or data is None:
        ...
        try:
            ...
            # Update timeseries graph and stats
            timeseries_fig = create_timeseries(df, selected_metric, time_period, smoothing_window)
            timeseries_stats = make_summary_stats(df, selected_metric, f"{metric_options.get(selected_metric, selected_metric)} Statistics")
            
            # Update gantt chart and stats
            if selected_night:
                mask = gantt_data["creationDate"].dt.date.astype(str) == selected_night
                this_night_df = gantt_data[mask].copy()
                gantt_fig = create_enhanced_gantt_one_night(this_night_df)
                gantt_stats = build_gantt_stats(selected_night, gantt_data, agg)
            else:
                gantt_fig = empty_fig
                gantt_stats = ""
            
            # Update sleep timing graph and stats
            timing_fig = create_sleep_timing_chart(df)
            timing_stats = make_timing_stats(df)
            
            # Update correlation matrix and explanation
            correlation_fig = create_correlation_matrix(df)
            correlation_explanation = dbc.Card([
                dbc.CardHeader("About Correlations"),
                dbc.CardBody([
